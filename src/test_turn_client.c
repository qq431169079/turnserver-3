...
static int client_recv_message(int transport_protocol, int sock, struct tls_peer* speer, char* buf, size_t buflen)
{
  ...
  if(transport_protocol == IPPROTO_UDP)
  {
	...
	//printf("IP:%s\n", (char *)(&((struct sockaddr_in *)&saddr)->sin_addr));
...
int main(int argc, char** argv)
{
  ...
  char port_str[8];
  uint8_t nonce[513] = "5953b082100000009a5109573ca8d075ad3e25a4ca773e0d";
  size_t nonce_len = 0;
  char user[16] = "toto";
  char password[16] = "password";
  char domain[16] = "domain.org";
  const uint16_t channel = 0x4009;
  char data[1024]= "fefefefefefefefefefefefefefefefefe";
  uint8_t family = 0;
  ...
  if(conf.username)
  {
    strncpy(user,conf.username,15);
  }

  if(conf.realm)
  {
  	strncpy(domain,conf.realm,15);
  }
  ...
  if(client_create_permission(transport_protocol, sock, speer, (struct sockaddr*)&server_addr, server_addr_size, (struct sockaddr*)&peer_addr, user, md_buf, domain, nonce, nonce_len) == -1)
  {
	...
	/*while(1)
	{
		...
		uint16_t tabu[16];
  		size_t tabu_size = sizeof(tabu) / sizeof(uint16_t);
fprintf(stderr, "test: while 0 end\n");
		client_recv_message(IPPROTO_UDP, sock, speer, buff, sizeof(buff));
		 if(nb == -1)
		{
			fprintf(stderr, "Receive failed!\n");
			return -1;
		}
fprintf(stderr, "test: while 1 end\n");

		if(turn_parse_message(buff, nb, &message, tabu, &tabu_size) == 0)
		{
				point = (char *)message.data->turn_attr_data;
				printf("Buff:%s\n",point);
		}else
		{
			printf("turn_parse_message error!Buff:%s\n",buff);
		}
		fprintf(stderr, "test: while end\n");			
	}
