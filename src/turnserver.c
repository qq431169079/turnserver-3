    ...
    struct tls_peer* speer)
{
  ...
  if(desc)
  {
    ...
    {
      //CMS_LOG_Printf("turnserver_send_error:437");
  ...
  */
  ...
  while(!has_token && (relayed_sock == -1 && quit_loop < 5))
  {
     ...
     */
  	...
  	//CMS_LOG_Printf("turn_attr_protocol:%d,str:%s,port:%d", message->requested_transport->turn_attr_protocol,str,port);
    ...
    struct tls_peer* speer)
{
  ...
  if(method != TURN_METHOD_ALLOCATE)
  {
	...
	////CMS_LOG_Printf("ClientIp:%s,ClientPort:%d",inet_ntop(((struct sockaddr_storage *)saddr)->ss_family, &((struct sockaddr_in *)&saddr)->sin_addr, addrbuf, INET6_ADDRSTRLEN),ntohs(((struct sockaddr_in*)saddr)->sin_port));
	////CMS_LOG_Printf("ServerIp:%s,serverPort:%d",inet_ntop(((struct sockaddr_storage *)daddr)->ss_family, &((struct sockaddr_in *)&daddr)->sin_addr, addrbuf, INET6_ADDRSTRLEN),ntohs(((struct sockaddr_in*)daddr)->sin_port));
	if(STUN_IS_REQUEST(hdr_msg_type))
    {
      ...
      if(desc && message->username && message->realm)
      {
        ...
        {
          //CMS_LOG_Printf("111111111111");
          desc = NULL;
        }
      }
      else
      {
      	//CMS_LOG_Printf("222222222");
        desc = NULL;
      }
    }
...
{
  //CMS_LOG_Printf("start turnserver ...");
  struct list_head allocation_list;
  struct list_head account_list;
  struct list_head* n = NULL;
  struct list_head* get = NULL;
